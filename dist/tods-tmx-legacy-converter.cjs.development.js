...
var todsCompetitionFactory = require('tods-competition-factory');
var d3 = require('d3');
...
function convertTieFormat(matchorder) {
  var matchUpTypes = todsCompetitionFactory.utilities.unique(matchorder.map(function (_ref) {
  ...
  var collectionDefinitions = matchUpTypes.map(function (matchUpType) {
    ...
    var collectionDefinition = {
      ...
      collectionId: todsCompetitionFactory.utilities.UUID(),
      collectionValue: collectionValue,
      matchUpType: matchUpType
    };
    var valuesDiffer = todsCompetitionFactory.utilities.unique(collectionValueProfile.map(function (_ref3) {
...
function extractMatchUp(_ref) {
    ...
    legacyMatch.teams.forEach(function (team, index) {
      ...
      if (participantId && !participantIds.includes(participantId)) {
        ...
        var entry = {
          ...
          entryStatus: todsCompetitionFactory.entryStatusConstants.DIRECT_ACCEPTANCE
        };
        entries.push(entry);
  ...
  var sets = todsCompetitionFactory.mocksEngine.parseScoreString({
  ...
  var matchUpStatus = live && todsCompetitionFactory.matchUpStatusConstants.IN_PROGRESS || interrupted && todsCompetitionFactory.matchUpStatusConstants.SUSPENDED || incomplete && todsCompetitionFactory.matchUpStatusConstants.INCOMPLETE || walkover && todsCompetitionFactory.matchUpStatusConstants.WALKOVER || cancelled && todsCompetitionFactory.matchUpStatusConstants.NOT_PLAYED || abandoned && todsCompetitionFactory.matchUpStatusConstants.ABANDONED || defaulted && todsCompetitionFactory.matchUpStatusConstants.DEFAULTED || retired && todsCompetitionFactory.matchUpStatusConstants.RETIRED || isBye && todsCompetitionFactory.matchUpStatusConstants.BYE || winningSide && todsCompetitionFactory.matchUpStatusConstants.COMPLETED || time && todsCompetitionFactory.matchUpStatusConstants.COMPLETED || !winningSide && todsCompetitionFactory.matchUpStatusConstants.TO_BE_PLAYED;
...
function getTimeItems(_ref2) {
  ...
  if (umpire) {
    var tournamentOfficials = participants == null ? void 0 : participants.filter(function (participant) {
      return participant.participantType === todsCompetitionFactory.participantConstants.INDIVIDUAL && participant.participantRole === todsCompetitionFactory.participantRoles.OFFICIAL;
    });
...
function getMatchUpType(format) {
  return ["S", "SINGLES"].includes(format.toUpperCase()) && todsCompetitionFactory.eventConstants.SINGLES || ["D", "DOUBLES"].includes(format.toUpperCase()) && todsCompetitionFactory.eventConstants.DOUBLES;
}
function getGender(value) {
  if (!value) return todsCompetitionFactory.genderConstants.MIXED;
  if (["F", "FEMALE", "W", "WOMAN"].includes(value.toUpperCase())) return todsCompetitionFactory.genderConstants.FEMALE;
  if (["M", "MALE", "MAN"].includes(value.toUpperCase())) return todsCompetitionFactory.genderConstants.MALE;
  return todsCompetitionFactory.genderConstants.MIXED;
}
function getStage(_ref) {
  var legacyEvent = _ref.legacyEvent;
  var stageMap = {
    E: todsCompetitionFactory.drawDefinitionConstants.MAIN,
    Q: todsCompetitionFactory.drawDefinitionConstants.QUALIFYING,
    S: todsCompetitionFactory.drawDefinitionConstants.MAIN,
    C: todsCompetitionFactory.drawDefinitionConstants.CONSOLATION,
    P: todsCompetitionFactory.drawDefinitionConstants.PLAY_OFF,
    A: todsCompetitionFactory.drawDefinitionConstants.MAIN
  };

  if (legacyEvent.draw_type === "R") {
    if (Object.keys(legacyEvent.links || {}).includes("E")) {
      return todsCompetitionFactory.drawDefinitionConstants.QUALIFYING;
    } else {
      return todsCompetitionFactory.drawDefinitionConstants.MAIN;
    }
  }
...
function drawFx(opts) {
  ...
  function bracketDrawPositions(draw) {
    ...
    return (_ref9 = []).concat.apply(_ref9, draw.brackets.map(function (b, i) {
      return d3.range(draw.bracket_size).map(function (p, j) {
        return {
          bracket: i,
          position: j + 1
        };
  ...
  function treeInfo(draw, collapse) {
    ...
    if (!draw) return {};
    var calc_tree = d3.tree();
    var draw_hierarchy = d3.hierarchy(draw);
  ...
  function buildQualDraw(_ref20) {
    ...
    var max_round = d3.hierarchy(children[0]).height;
    return {
      children: children,
      max_round: max_round
    };
  ...
  function roundrobinSeedPlacements(_ref29) {
    ...
    d3.range(auto_placed_seeds.length).forEach(function (s) {
    ...
    var range = [];
    var positions = [];
    d3.range(bracket_count).forEach(function (s) {
  ...
  function qualifyingBracketSeeding(_ref30) {
    ...
    var range = [];
    var positions = [];
    d3.range(random_placed_seeds.length).forEach(function (s) {
  ...
  function rrByeDistribution(_ref37) {
    ...
    draw.bye_placements = d3.range(byes).map(function (b, i) {
  ...
  function distributeQualifiers(_ref42) {
    ...
    var qualifiers = d3.range(0, num_qualifiers).map(function () {
      return [{
        entry: "Q",
        qualifier: true
      }];
    ...
    var sections = d3.range(0, Math.floor(total / section_size)); // all draw positions which have a first-round opponent (no structural bye);
    ...
    d3.range(0, num_qualifiers).forEach(function () {
      var section = randomPop(sections);
      var dprange = d3.range(section * section_size + 1, section * section_size + section_size + 1);
  ...
  function placeSeedGroups(_ref45) {
    ...
    d3.range(0, count).forEach(function () {
...
var CONTAINER = todsCompetitionFactory.drawDefinitionConstants.CONTAINER,
    ITEM = todsCompetitionFactory.drawDefinitionConstants.ITEM,
    ROUND_OUTCOME = todsCompetitionFactory.drawDefinitionConstants.ROUND_OUTCOME,
    WIN_RATIO = todsCompetitionFactory.drawDefinitionConstants.WIN_RATIO;
...
function processLegacyMatch(_ref4) {
  ...
  var tieMatchUps = tieMatches.filter(function (tieMatch) {
    ...
    var _extractMatchUp = extractMatchUp({
      ...
      participants: participants,
      participantIds: participantIds,
      tournamentEngine: todsCompetitionFactory.tournamentEngine,
...
function extractStructures(_ref) {
  ...
  var structures = legacyEvents.map(function (legacyEvent) {
    ...
    var stage = legacyEvent.euid === mainStructureId ? todsCompetitionFactory.drawDefinitionConstants.MAIN : getStage({
      legacyEvent: legacyEvent
    });
    var structure = {
      stage: stage,
...
function extractEvents(_ref) {
  ...
  todsCompetitionFactory.tournamentEngine.setState(tournamentRecord); // linkedStructures are events which have explicit links
  ...
  Object.keys(linkedStructures).forEach(function (key) {
    ...
    var eventType = getMatchUpType(mainLegacyEvent.format) || (mainLegacyEvent.matchorder || tournament.type === "dual") && todsCompetitionFactory.matchUpTypes.TEAM;
    ...
    var drawDefinition = {
      ...
      drawId: todsCompetitionFactory.utilities.UUID(),
      drawName: custom_category || broadcast_name || name || todsCompetitionFactory.factoryConstants.drawDefinitionConstants.MAIN,
    ...
    todsCompetitionFactory.tournamentEngine.addDrawDefinitionExtension({
      drawDefinition: drawDefinition,
      extension: extension
    });
    var eventId = todsCompetitionFactory.utilities.UUID();
...
function extractTeamParticipants(_ref2) {
  ...
  var teamParticipants = (tournament.teams || []).map(function (team) {
    ...
    var teamParticipant = {
      ...
      participantType: todsCompetitionFactory.participantConstants.TEAM,
      participantRole: todsCompetitionFactory.participantRoles.COMPETITOR,
  ...
  relevantEvents.forEach(function (legacyEvent) {
    ...
    teams.filter(function (team) {
      ...
      if (individualParticipants.length === 2) {
        ...
        var pairParticipant = {
          participantId: todsCompetitionFactory.utilities.UUID(),
          participantType: todsCompetitionFactory.participantConstants.PAIR,
          participantRole: todsCompetitionFactory.participantRoles.COMPETITOR,
          individualParticipantIds: individualParticipantIds,
...
function extractIndividualParticipants(_ref4) {
  ...
  var tournamentStartDate = tournament.start && dateFns.format(new Date(tournament.start), "yyyy-MM-dd");
  ...
  function addParticipant(player) {
    ...
    var birthDate = isValidDate(player.birth) && dateFns.format(new Date(player.birth), "yyyy-MM-dd");
    var participant = {
      participantName: participantName,
      participantId: participantId,
      participantType: todsCompetitionFactory.participantConstants.INDIVIDUAL,
      participantRole: todsCompetitionFactory.participantRoles.COMPETITOR,
      timeItems: [],
      person: {
...
function isValidDate(date) {
  ...
  try {
    ...
    if (dateObject.trim() === todsCompetitionFactory.errorConditionConstants.INVALID_DATE) {
      return false;
    }
...
function getName(text) {
  return normalizeText.normalizeName(text || "", ["de", "la", "da"]);
}
...
function addRankings(_ref7) {
  ...
  if (player.rankings) {
    Object.keys(player.rankings).forEach(function (key) {
      var itemType = todsCompetitionFactory.scaleConstants.SCALE + "." + todsCompetitionFactory.scaleConstants.RANKING + ".SINGLES." + key;
      var timeItem = {
        itemType: itemType,
  ...
  if (player.category_dbls && tournamentCategory) {
    var itemType = todsCompetitionFactory.scaleConstants.SCALE + "." + todsCompetitionFactory.scaleConstants.RANKING + ".SINGLES." + tournamentCategory;
    var timeItem = {
      itemType: itemType,
  ...
  if (player.ratings) {
    Object.keys(player.ratings).forEach(function (key) {
      Object.keys(player.ratings[key]).forEach(function (ratingType) {
        var itemType = todsCompetitionFactory.scaleConstants.SCALE + "." + todsCompetitionFactory.scaleConstants.RATING + "." + ratingType.toUpperCase() + "." + key.toUpperCase();
        var timeItem = {
          itemType: itemType,
...
function addSignInStatus(_ref9) {
  ...
  var itemValue = player.signed_in ? todsCompetitionFactory.participantConstants.SIGNED_IN : todsCompetitionFactory.participantConstants.SIGNED_OUT;
  var timeItem = {
    itemSubject: todsCompetitionFactory.participantConstants.SIGN_IN_STATUS,
    timeStamp: tournamentStartDate,
    itemValue: itemValue
  };
  participant.timeItems.push(timeItem);
...
function addPenalties(_ref10) {
  ...
  if (player.penalties) {
    ...
    player.penalties.forEach(function (penalty) {
      ...
      var penaltyId = todsCompetitionFactory.utilities.UUID();
      var penaltyItem = {
        penaltyId: penaltyId,
  ...
  function getPenaltyType(penalty) {
    ...
    if (((_penalty$penalty2 = penalty.penalty) == null ? void 0 : _penalty$penalty2.value) === "unsporting") return todsCompetitionFactory.penaltyConstants.UNSPORTSMANLIKE_CONDUCT;
    if (((_penalty$penalty3 = penalty.penalty) == null ? void 0 : _penalty$penalty3.value) === "fail2signout") return todsCompetitionFactory.penaltyConstants.FAILURE_TO_COMPLETE;
    if (((_penalty$penalty4 = penalty.penalty) == null ? void 0 : _penalty$penalty4.value) === "illegalcoaching") return todsCompetitionFactory.penaltyConstants.COACHING;
    if (((_penalty$penalty5 = penalty.penalty) == null ? void 0 : _penalty$penalty5.value) === "ballabuse") return todsCompetitionFactory.penaltyConstants.BALL_ABUSE;
    if (((_penalty$penalty6 = penalty.penalty) == null ? void 0 : _penalty$penalty6.value) === "racquetabuse") return todsCompetitionFactory.penaltyConstants.RACKET_ABUSE;
    if (((_penalty$penalty7 = penalty.penalty) == null ? void 0 : _penalty$penalty7.value) === "equipmentabuse") return todsCompetitionFactory.penaltyConstants.EQUIMENT_VIOLATION;
    if (((_penalty$penalty8 = penalty.penalty) == null ? void 0 : _penalty$penalty8.value) === "cursing") return todsCompetitionFactory.penaltyConstants.UNSPORTSMANLIKE_CONDUCT;
    if (((_penalty$penalty9 = penalty.penalty) == null ? void 0 : _penalty$penalty9.value) === "rudegestures") return todsCompetitionFactory.penaltyConstants.UNSPORTSMANLIKE_CONDUCT;
    if (((_penalty$penalty10 = penalty.penalty) == null ? void 0 : _penalty$penalty10.value) === "foullanguage") return todsCompetitionFactory.penaltyConstants.UNSPORTSMANLIKE_CONDUCT;
    if (((_penalty$penalty11 = penalty.penalty) == null ? void 0 : _penalty$penalty11.value) === "timeviolation") return todsCompetitionFactory.penaltyConstants.PUNCTUALITY;
    if (((_penalty$penalty12 = penalty.penalty) == null ? void 0 : _penalty$penalty12.value) === "latearrival") return todsCompetitionFactory.penaltyConstants.PUNCTUALITY;
  }
}
...
function extractTournamentInfo(_ref) {
  ...
  var tournamentInfo = {
    ...
    startDate: tournament.start && new Date(dateFns.format(new Date(tournament.start), "yyyy-MM-dd")).toISOString(),
    endDate: tournament.end && new Date(dateFns.format(new Date(tournament.end), "yyyy-MM-dd")).toISOString(),
...
function getRefereeParticipant(referee) {
  ...
  var participantId = todsCompetitionFactory.utilities.UUID();
  return {
    name: referee,
    participantId: participantId,
    participantType: todsCompetitionFactory.participantConstants.INDIVIDUAL,
    participantRole: todsCompetitionFactory.participantRoles.OFFICIAL,
    person: {
      personId: participantId,
      standardFamilyName: standardFamilyName,
      standardGivenName: standardGivenName
...
exports.default = getTournamentRecordTODS;
exports.getTournamentRecordTODS = getTournamentRecordTODS;
//# sourceMappingURL=tods-tmx-legacy-converter.cjs.development.js.map
